...
document.getElementById('SavedDatasButton').addEventListener('click', function() {
    ...
    if (savedData.length > 0) {
        savedData.forEach((item, index) => {
            ...
            // データアイテムに削除ボタンを追加
            dataItem.innerHTML = `
                <span><strong>Before:</strong> ${item.inputValue} (${item.inputBase})</span>
                <span><strong>After:</strong> ${item.outputValue} (${item.outputBase})</span>
                <span><small>Saved Time: ${formattedDate}</small></span>
        ...
        savedDataList.innerHTML = '<p>No saved data</p>';
    }
